...
And honestly, that mindset is [completely normal](#yellow). Every developer begins like that. But as time goes on, you eventually realize that simply [making code run](#underline) is not enough. The real goal is to make it [run efficiently](#green).

To understand why this matters, let’s look at a simple LeetCode problem: removeElement.

### The Problem

You are given an array `nums` and a value `val`. The task is to remove all elements equal to `val` [in-place](#purple) and return the number of remaining elements.

It’s a [simple problem](#yellow) on the surface, but the way you solve it says a lot about your understanding of [algorithm efficiency](#green).
...
- [Extremely slow](#yellow) on large arrays
- Using `splice` forces the array to [shift elements](#purple) each time
- Leads to [O(n²) time complexity](#green)
- [Not suitable](#underline) for interviews or production-level code

**_Here is the Big O chart that this code executes._**
...
- Runs in [O(n) time](#green)
- Uses [O(1) extra space](#green)
- [Efficient](#yellow) even for very large input sizes
- [Preferred solution](#underline) in interviews
- Good example of [algorithmic thinking](#purple)
...
[Big O](#green) matters because it helps you understand how your code behaves as [data grows](#yellow). Your program might work perfectly with 50 elements, but [break completely](#underline) with 50,000 elements.
